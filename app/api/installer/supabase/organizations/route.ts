...
import { listSupabaseOrganizations } from '@/lib/installer/edgeFunctions';
...
export async function POST(req: Request) {
  ...
  const res = await listSupabaseOrganizations({ accessToken: parsed.data.accessToken.trim() });
  if (!res.ok) {
    return json({ error: res.error, status: res.status }, res.status || 500);
  }

  return json({ ok: true, organizations: res.organizations });
}
