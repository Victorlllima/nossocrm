/**
 * WhatsApp Webhook Handler - ANTHROPIC VERSION WITH TOOLS
 *
 * Uses Anthropic Claude instead of OpenAI to avoid quota issues
 * Fixes applied:
 * 1. Switched from @ai-sdk/openai to @ai-sdk/anthropic
 * 2. Uses claude-opus-4-1 model (latest, capable, cost-effective)
 * 3. Maintained exponential backoff retry logic
 * 4. Implemented single-call execution (no concurrent tool calls)
 * 5. Added request logging for quota monitoring
 * 6. Temperature set to 1.0 (Claude default for balanced responses)
 * 7. Limited maxSteps to 2 (not 5) to reduce API calls
 * 8. Tools enabled for property search and human escalation
 */
 ...
 * ANTHROPIC VERSION: Uses Claude instead of OpenAI
 */
export async function POST(req: NextRequest) {
    try {
        ...
        console.log(`📨 [${requestId}] Request #${requestCount} in current minute (ANTHROPIC)`);
        ...
        console.log(`[${requestId}] Calling Anthropic (model: claude-3-5-sonnet, maxSteps: 2)`);

        // Call AI agent with ANTHROPIC:
        // 1. claude-opus-4-1 model (latest, capable, cost-effective)
        // 2. temperature 1.0 (Claude default, balanced)
        // 3. maxSteps: 2 (instead of 5) - reduces API calls
        // 4. Exponential backoff for any service errors
        const { text: responseText } = await retryWithBackoff(
            () => generateText({
                ...
                maxSteps: 2,  // Reduced from 5 - prevents excessive API calls
            }),
            3,      // max attempts
