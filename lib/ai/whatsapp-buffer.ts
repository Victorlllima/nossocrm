...
const BUFFER_WAIT_MS = 5000; // 5 seconds - prevents rapid message loops while allowing natural conversation
...
/**
 * Checks if there are pending buffered messages for a phone
 * Used to decide if webhook should retry processing
 *
 * @param phone - Lead's phone number
 * @returns true if there are buffered messages waiting
 */
export function hasBufferedMessages(phone: string): boolean {
    const entry = messageBuffer.get(phone);
