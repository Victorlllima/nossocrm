...
export async function sendWhatsAppMessage(phone: string, text: string): Promise<boolean> {
    try {
        ...
        // Format phone for Evolution API (remove @s.whatsapp.net, send just number)
        const formattedPhone = phone.replace('@s.whatsapp.net', '').replace('@', '');

        console.log(`📤 Sending to Evolution API: ${formattedPhone} (${text.substring(0, 50)}...)`);
        ...
        console.log(`✅ Evolution API success: ${formattedPhone}`);
        return true;
 ...
 * @param sessionId - Optional session ID for logging (not used)
 * @returns Success status
 */
export async function formatAndSendResponse(phone: string, responseText: string, sessionId?: string): Promise<boolean> {
