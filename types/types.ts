...
export interface Contact {
  ...
  createdAt: string;
  updatedAt?: string; // Íšltima modificação do registro
...
export interface Activity {
  ...
  organizationId?: OrganizationId; // Tenant FK (for RLS) - optional during migration
  dealId: string;
  /** ID do contato associado (opcional). Íštil para tarefas sem deal. */
  contactId?: string;
...
export interface PaginationState {
  /** Índice da página atual (0-indexed). */
  pageIndex: number;
...
export interface PaginatedResponse<T> {
  ...
  totalCount: number;
  /** Índice da página retornada (0-indexed). */
  pageIndex: number;
